...
export function FactsTab(props: { type?: string; theme?: (type: string) => EntityTheme }) {
  ...
  const facts = createMemo(() =>
    props.type ? store.facts.filter((f) => matchesType(types().get(f.e) ?? "unknown", props.type!)) : store.facts,
  )
  const filtered = createMemo(() => {
    ...
    if (q.startsWith("entity:")) {
      ...
      return facts().filter((f) => f.e.toLowerCase().includes(v))
    }
    if (q.startsWith("attr:")) {
      const v = q.slice(5).trim()
      return facts().filter((f) => f.a.toLowerCase().includes(v))
    }
    if (q.startsWith("value:")) {
      const v = q.slice(6).trim()
      return facts().filter((f) => String(f.v).toLowerCase().includes(v))
    }
    return facts().filter(
      (f) => f.e.toLowerCase().includes(q) || f.a.toLowerCase().includes(q) || String(f.v).toLowerCase().includes(q),
    )
  })
      ...
      <div class="shrink-0 flex flex-col gap-1.5 border-b border-border-weaker-base px-2 py-1.5 sm:flex-row sm:items-center sm:gap-2 sm:px-3 sm:py-2">
        <div class="flex w-full items-center gap-1.5 rounded-md bg-surface-raised-base/40 px-2 py-0.5 sm:flex-1 sm:min-w-0 sm:py-1">
          ...
          <input
            ...
            placeholder="Filter…   entity: attr: value:"
            value={query()}
            ...
            <Show
              ...
              fallback={
                <div class="h-full flex items-center justify-center text-12-regular text-text-weaker">
                  No facts{props.type ? ` for ${props.type}` : ""}
                </div>
              }
            >
              <div
              ...
              >
                <div class="sticky top-0 z-10 hidden grid-cols-[minmax(12rem,1.25fr)_minmax(10rem,0.75fr)_minmax(16rem,1.5fr)] bg-background-base text-text-weaker text-10-medium uppercase tracking-wide sm:grid">
                  <div class="px-3 py-2 font-medium">Entity</div>
                  <div class="px-3 py-2 font-medium">Attribute</div>
                  <div class="px-3 py-2 font-medium">Value</div>
                </div>
                <div class="relative text-12-regular" style={{ height: `${filtered().length * row()}px` }}>
                  <For each={visible()}>
                    {(f, i) => (
                      <div
                        class="absolute left-2 right-2 flex flex-col justify-center gap-1 rounded-lg border border-border-weaker-base/50 px-3 py-2 hover:bg-surface-raised-base/30 sm:left-0 sm:right-0 sm:grid sm:grid-cols-[minmax(12rem,1.25fr)_minmax(10rem,0.75fr)_minmax(16rem,1.5fr)] sm:rounded-none sm:border-x-0 sm:border-b-0 sm:px-0 sm:py-0"
                        style={{
                          top: `${(range().start + i()) * row()}px`,
                          height: `${row()}px`,
                        }}
                      >
                        <div class="font-mono min-w-0 sm:px-3 sm:py-1.5">
                          <button
                            class="text-text-base hover:underline truncate block max-w-full"
                            style={{ color: theme(typeOf(f.e)).color }}
                            onClick={() => dialog.push(f.e, typeOf(f.e))}
                          >
                            {f.e}
                          </button>
                        </div>
                        <div class="text-text-weak font-mono truncate sm:px-3 sm:py-1.5">{f.a}</div>
                        <div class="text-text-base font-mono truncate sm:px-3 sm:py-1.5">{String(f.v)}</div>
                      </div>
                    )}
                  </For>
                </div>
