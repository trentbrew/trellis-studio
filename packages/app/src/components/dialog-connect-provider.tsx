...
import { CODEX_PREFERRED_MODEL_IDS, OPENAI_PROVIDER_ID, preferredCodexModelKey } from "@/lib/codex-model"
...
export function DialogConnectProvider(props: { provider: string }) {
  ...
  const visibleMethods = createMemo(() => {
    const list = methods()
    if (props.provider !== "openai") return list
    if (server.isLocal()) return list
    return list.filter((item) => !(item.type === "oauth" && item.label.toLowerCase().includes("browser")))
  })

  const oauthContext = createMemo((): Record<string, string> => ({
    remote: server.isLocal() ? "false" : "true",
  }))

  function methodIndex(visibleIndex: number) {
    const item = visibleMethods()[visibleIndex]
    if (!item) return visibleIndex
  ...
  async function selectMethod(index: number, inputs?: Record<string, string>) {
    ...
    const resolved = methodIndex(index)
    const method = methods()[resolved]
    dispatch({ type: "method.select", index: resolved })
  ...
  async function complete() {
    const oauth = props.provider === OPENAI_PROVIDER_ID && method()?.type === "oauth"
    await globalSDK.client.global.dispose()

    if (oauth) {
      const openai = providers.all().find((item) => item.id === OPENAI_PROVIDER_ID)
      const model = preferredCodexModelKey(openai?.models ?? {}) ?? {
        providerID: OPENAI_PROVIDER_ID,
        modelID: CODEX_PREFERRED_MODEL_IDS[0],
      }
      models.recent.push(model)
    ...
    showToast({
      ...
      icon: "circle-check",
      title: oauth
        ? language.t("provider.connect.toast.openaiCodex.title")
        : language.t("provider.connect.toast.connected.title", { provider: provider().name }),
      description: oauth
        ? language.t("provider.connect.toast.openaiCodex.description")
  ...
  function MethodSelection() {
    return (
      <>
        ...
        <Show when={props.provider === "openai" && !server.isLocal()}>
          <div class="text-14-regular text-text-weaker">
            {language.t("provider.connect.openai.remoteHint")}
          </div>
        </Show>
        <div>
          <List
