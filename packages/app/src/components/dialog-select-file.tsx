...
function createFileEntries(props: {
  ...
  const root = createMemo(() => {
    ...
    return paths.slice(0, props.limit()).map((path) => createFileEntry(path, category))
  })
...
export function DialogSelectFile(props: {
  ...
  const fileEntries = createFileEntries({
    ...
    tabs,
    language,
    limit: () => (filesOnly() ? FILES_ENTRY_LIMIT : ENTRY_LIMIT),
  })
  ...
  const items = async (text: string) => {
    const query = text.trim()
    setGrouped(query.length > 0 && !filesOnly())
  ...
  return (
    <Dialog class="pt-3 pb-0 !max-h-[480px]" transition>
      <List
        ...
        skipFilter={(filter) => filesOnly() && filter.trim().length > 0}
