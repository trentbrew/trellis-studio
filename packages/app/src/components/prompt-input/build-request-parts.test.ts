...
describe("buildRequestParts", () => {
  test("builds typed request and optimistic parts without cast path", () => {
    ...
    const foo = result.requestParts.find(
    ...
    expect(foo).toMatchObject({ type: "file", mime: "text/plain" })
  ...
  test("keeps explicit pdf references as application/pdf", () => {
    const prompt: Prompt = [{ type: "file", path: "docs/guide.pdf", content: "@docs/guide.pdf", start: 0, end: 15 }]

    const result = buildRequestParts({
      prompt,
      context: [{ key: "ctx_pdf", type: "file", path: "docs/spec.pdf" }],
      images: [],
      text: "@docs/guide.pdf",
      messageID: "msg_pdf",
      sessionID: "ses_pdf",
    ...
    expect(files).toMatchObject([
      {
        ...
        url: "file:///repo/docs/guide.pdf",
        filename: "guide.pdf",
      },
      {
        type: "file",
        mime: "application/pdf",
        url: "file:///repo/docs/spec.pdf",
        filename: "spec.pdf",
      },
    ])
  })

  test("keeps media references with their native mime", () => {
    const prompt: Prompt = [
      { type: "file", path: "media/voice.mp3", content: "@media/voice.mp3", start: 0, end: 16 },
      { type: "file", path: "media/clip.mp4", content: "@media/clip.mp4", start: 17, end: 32 },
    ]
    ...
    const result = buildRequestParts({
      ...
      context: [],
      images: [],
      text: "@media/voice.mp3 @media/clip.mp4",
      messageID: "msg_media",
      sessionID: "ses_media",
      sessionDirectory: "/repo",
    })
    ...
    expect(files).toMatchObject([
      { type: "file", mime: "audio/mpeg", filename: "voice.mp3" },
      { type: "file", mime: "video/mp4", filename: "clip.mp4" },
    ])
  })
