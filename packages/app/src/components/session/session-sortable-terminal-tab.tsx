...
export function SortableTerminalTab(props: { terminal: LocalPTY; onClose?: () => void }): JSX.Element {
  ...
  const splitMode = () => !!terminal.paneTree()

  const close = () => {
    const tree = terminal.paneTree()
    if (tree && ptyInPaneTree(tree, props.terminal.id)) {
      terminal.unsplit(props.terminal.id)
      return
    }
  ...
  const focus = () => {
    ...
    terminal.focusPane(props.terminal.id)
  ...
  return (
    <div
      use:sortable
      class="outline-none focus:outline-none focus-visible:outline-none flex items-center px-2.5"
      classList={{
        "h-full": true,
    ...
    >
      <div class="relative h-full flex items-center">
        <Tabs.Trigger
          ...
          onContextMenu={menu}
          class="!shadow-none px-1.5"
          classes={{
            button:
              "border-0 outline-none focus:outline-none focus-visible:outline-none !shadow-none !ring-0 flex items-center",
          }}
          closeButton={
            <IconButton
              icon="close-small"
              ...
              aria-label={splitMode() ? language.t("command.terminal.pane.close") : language.t("terminal.close")}
            />
          }
        >
          <Icon name="terminal" class="w-4 h-4" />
          <div class="flex items-center gap-1 text-[12px] ml-1">
          ...
          <DropdownMenu.Portal>
            ...
            >
              ...
              <DropdownMenu.Item onSelect={close}>
                ...
                {splitMode() ? language.t("command.terminal.pane.close") : language.t("common.close")}
              </DropdownMenu.Item>
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        </DropdownMenu>
