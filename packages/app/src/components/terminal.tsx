...
export const Terminal = (props: TerminalProps) => {
  ...
  const fitWithMargin = () => {
    ...
    if (term && term.rows > 1) {
      ...
      // Re-anchor viewport to bottom after resize
      const anchorLine = Math.max(0, term.buffer.active.length - term.rows)
      term.scrollToLine(anchorLine)
    }
  }
  ...
  onMount(() => {
    ...
    const run = async () => {
      ...
      const checkAtBottom = () => {
        const viewportY = t.getViewportY()
        const totalRows = t.buffer.active.length
        setAtBottom(viewportY + t.rows >= totalRows)
      }
      const onScroll = t.onScroll(() => checkAtBottom())
      ...
      if (restore && restoreSize) {
        ...
        if (restore) {
          ...
          // Pre-fill blank lines so the cursor (and first PTY output) anchors to the bottom
          t.write("\n".repeat(t.rows - 1))
        }
        startResize()
      }
      ...
      const open = () => {
        ...
        if (auth?.password) {
          next.username = auth.username ?? "opencode"
        ...
        const handleOpen = () => {
          ...
          local.onConnect?.()
          scheduleSize(t.cols, t.rows)
          if (firstConnect && local.clearOnConnect && !restore) {
            firstConnect = false
            socket.send("clear\r")
          }
        }
    ...
    <div class="relative size-full overflow-hidden">
      <div
        ...
        classList={{
          ...(local.classList ?? {}),
          "select-text": true,
          "size-full pl-4 pb-0 pt-0 font-mono relative overflow-hidden": true,
