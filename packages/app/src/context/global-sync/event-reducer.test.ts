...
describe("applyDirectoryEvent", () => {
  ...
  test("cleanupDroppedSessionCaches preserves active session from window location", () => {
    const activeMsg = userMessage("msg_active", "ses_active")
    const [store, setStore] = createStore(
      baseState({
        session: [rootSession({ id: "ses_keep" })],
        message: {
          ses_active: [activeMsg],
          ses_drop: [userMessage("msg_drop", "ses_drop")],
        },
      }),
    )
    ...
    ;(globalThis as any).window = {
      location: {
        pathname: "/dir/session/ses_active",
      },
    }
    ...
    expect(store.message.ses_active).toEqual([activeMsg])
    expect(store.message.ses_drop).toBeUndefined()
  })
