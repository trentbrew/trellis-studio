...
import { isOpenaiCodexSubscription, preferredCodexModelKey } from "@/lib/codex-model"
import { hostedFreeModelKey, hostedZenFallbackModelKey } from "@/lib/opencode-zen-model"
...
export const {
  ...
  init: () => {
    ...
    const codexModel = () => {
      const openai = providers.all().find((item) => item.id === "openai")
      if (!openai || !connected().has("openai")) return
      if (!isOpenaiCodexSubscription(openai.models)) return
      const model = preferredCodexModelKey(openai.models)
      if (!model || !validModel(model)) return
      return model
    }

    const hostedZenModel = () => {
      if (server.isLocal()) return

      const hosted = hostedFreeModelKey()
      if (connected().has(hosted.providerID) && validModel(hosted)) return hosted

      const zen = hostedZenFallbackModelKey()
      if (!connected().has(zen.providerID)) return
      if (!validModel(zen)) return
      return zen
    }
    ...
    const defaultModel = () => {
      ...
      const cfg = Object.keys(sync.data.config.provider ?? {})
      const rank = (id: string) => {
        const idx = cfg.indexOf(id)
        if (idx >= 0) return idx
        const pop = popularProviders.indexOf(id)
        return cfg.length + (pop >= 0 ? pop : popularProviders.length)
      }
      const list = items.slice().sort((a, b) => rank(a.id) - rank(b.id))
      for (const provider of list) {
        if (provider.id === "openai" && isOpenaiCodexSubscription(provider.models)) {
          const model = preferredCodexModelKey(provider.models)
    ...
    const fallback = createMemo<ModelKey | undefined>(() => {
      ...
      const hosted = hostedZenModel()
      const recent = recentModel()
      const defaults = defaultModel()
      if (server.isLocal()) {
        return configured ?? recent ?? codex ?? defaults
      }
      return configured ?? codex ?? hosted ?? recent ?? defaults
    })
