...
export const { use: useNotification, provider: NotificationProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const unsub = globalSDK.event.listen((e) => {
      ...
      if (event.type === "cron.run.finished") {
        const props = event.properties as { id: string; jobId: string; reason: string; status: "ok" | "error" }
        const status = props.status === "ok" ? "success" : "error"
        const title = props.status === "ok" ? "Cron job completed" : "Cron job failed"
        const description = props.reason === "schedule" ? "Scheduled run" : "Manual run"
        showToast({ variant: status, title, description })
        return
      }
