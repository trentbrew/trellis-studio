...
import { ActiveTaskProvider } from "@/context/active-task"
...
import { LocalWorkspace, SortableWorkspace, type WorkspaceSidebarContext } from "./layout/sidebar-workspace"
...
export default function Layout(props: ParentProps) {
  const [store, setStore, , ready] = persisted(
    ...
    createStore({
      ...
      panelPosition: "bottom" as "bottom" | "right" | "left",
      panelAlignment: "center" as "center" | "justify",
    }),
  )
  ...
  const createSession = (directory: string | undefined) => {
    if (!directory) return
    clearSidebarHoverState()
    ...
    void client.session
      ...
      .then((session) => {
        if (!session) return
        const root = rememberSessionRoute(directory, session.id)
        const [store, setStore] = globalSync.child(directory, { bootstrap: false })
        if (!store.session.some((item) => item.id === session.id)) {
          setStore("session", (list) => [...list, session])
          setStore("sessionTotal", (count) => Math.max(count, store.sessionTotal + 1))
        }
        syncSessionRoute(directory, session.id, root)
        void client.session.laneActivate({ sessionID: session.id, directory }).catch(() => {})
        navigate(sessionRouteHref(slug, session.id))
        layout.mobileSidebar.hide()
      })
      .catch((err) => {
        notifications.push(
          `${language.t("prompt.toast.sessionCreateFailed.title")}: ${errorMessage(err, language.t("common.requestFailed"))}`,
          "toast",
        )
      })
  }
  ...
  function cycleTheme(direction = 1) {
    ...
    notifications.push(`${language.t("toast.theme.title")}: ${theme.name(nextThemeId)}`, "toast")
  }
  ...
  function cycleColorScheme(direction = 1) {
    ...
    notifications.push(`${language.t("toast.scheme.title")}: ${colorSchemeLabel(next)}`, "toast")
  }
  ...
  function setLocale(next: Locale) {
    ...
    notifications.push(
      `${language.t("toast.language.title")}: ${language.t("toast.language.description", { language: language.label(next) })}`,
      "toast",
    )
  }
  ...
  function adjustFontSize(delta: number) {
    ...
    settings.appearance.setFontSize(Math.max(8, Math.min(72, current + delta)))
  }

  function resetFontSize() {
    settings.appearance.setFontSize(14)
  }
    ...
    onMount(() => {
      ...
      const pollUpdate = () =>
        platform.checkUpdate!().then(({ updateAvailable, version }) => {
          ...
          notifications.push(
            `${language.t("toast.update.title")}: ${language.t("toast.update.description", { version: version ?? "" })}`,
            "toast",
          )
        })
  ...
  const useSDKNotificationToasts = () =>
    onMount(() => {
      ...
      const unsub = globalSDK.event.listen((e) => {
        ...
        notifications.push(`${title}: ${description}`, "toast")
      })
      onCleanup(unsub)
      ...
      createEffect(() => {
        ...
        for (const child of childSessions) {
          alertedAtBySession.delete(`${currentDir()}:${child.id}`)
        }
      })
    })
  ...
  command.register("layout", () => {
    const commands: CommandOption[] = [
      ...
      },
      {
        ...
        title: language.t("command.settings.shortcuts"),
        category: language.t("command.category.settings"),
        keybind: "mod+shift+k",
        onSelect: () => {
          const run = ++dialogRun
          void import("@/components/dialog-settings").then((x) => {
            ...
            dialog.show(() => <x.DialogSettings tab="shortcuts" />)
          })
        },
      },
      {
      ...
      },
      {
        ...
        title: language.t("command.view.toggleMinimap"),
        category: language.t("command.category.view"),
        keybind: "mod+shift+m,mod+alt+m",
        onSelect: () => setStore("minimapVisible", (v) => !v),
      },
      {
        id: "view.toggleMaximizedPanel",
        title: language.t("command.view.toggleMaximizedPanel"),
        category: language.t("command.category.view"),
        keybind: "ctrl+tab",
        onSelect: () => setStore("panelMaximized", (v) => !v),
      },
      {
        id: "view.toggleActivityBar",
        title: language.t("command.view.toggleActivityBar"),
        category: language.t("command.category.view"),
        keybind: "mod+shift+b",
        onSelect: () => setStore("activityBarVisible", (v) => !v),
      },
      {
        id: "view.moveSidebarLeft",
        title: language.t("command.view.moveSidebarLeft"),
        category: language.t("command.category.view"),
      ...
      {
        id: "view.moveSidebarRight",
        title: language.t("command.view.moveSidebarRight"),
        category: language.t("command.category.view"),
      ...
      {
        id: "view.alignPanelCenter",
        title: language.t("command.view.alignPanelCenter"),
        category: language.t("command.category.view"),
        keybind: "ctrl+shift+down",
        onSelect: () => setStore("panelAlignment", "center"),
      },
      {
        id: "view.positionPanelBottom",
        title: language.t("command.view.positionPanelBottom"),
        category: language.t("command.category.view"),
        keybind: "ctrl+shift+down",
        onSelect: () => setStore("panelPosition", "bottom"),
      },
      {
        id: "view.positionPanelRight",
        title: language.t("command.view.positionPanelRight"),
        category: language.t("command.category.view"),
        keybind: "ctrl+shift+right",
        onSelect: () => setStore("panelPosition", "right"),
      },
      {
        id: "view.positionPanelLeft",
        title: language.t("command.view.positionPanelLeft"),
        category: language.t("command.category.view"),
        keybind: "ctrl+shift+left",
        onSelect: () => setStore("panelPosition", "left"),
      },
      {
        id: "app.quit",
        title: language.t("command.app.quit"),
        category: language.t("command.category.app"),
        keybind: "mod+alt+q",
        onSelect: () => {
          /* platform.quit? */
        },
      ...
      {
        id: "app.reload",
        title: language.t("command.app.reload"),
        category: language.t("command.category.app"),
        keybind: "mod+shift+alt+r",
        onSelect: () => window.location.reload(),
      },
      {
        id: "app.restart",
        title: language.t("command.app.restart"),
        category: language.t("command.category.app"),
        keybind: "mod+shift+r",
        onSelect: () => {
      ...
      {
        id: "theme.toggleLightDark",
        title: language.t("command.theme.toggleLightDark"),
        category: language.t("command.category.theme"),
        keybind: "mod+shift+alt+t",
        onSelect: () => cycleColorScheme(1),
      },
      {
        id: "editor.fontZoomIn",
        title: language.t("command.editor.fontZoomIn"),
        category: language.t("command.category.editor"),
        keybind: "mod+alt+=",
        onSelect: () => adjustFontSize(1),
      },
      {
        id: "editor.fontZoomOut",
        title: language.t("command.editor.fontZoomOut"),
        category: language.t("command.category.editor"),
        keybind: "mod+alt+-",
        onSelect: () => adjustFontSize(-1),
      },
      {
        ...
        title: language.t("command.editor.fontZoomReset"),
        category: language.t("command.category.editor"),
        keybind: "alt+0",
      ...
      },
      {
        ...
        title: language.t("command.terminal.fontZoomIn"),
        category: language.t("command.category.terminal"),
        keybind: "mod+ctrl+alt+=",
        onSelect: () => adjustFontSize(1),
      },
      {
        id: "terminal.fontZoomOut",
        title: language.t("command.terminal.fontZoomOut"),
        category: language.t("command.category.terminal"),
        keybind: "mod+ctrl+alt+-",
        onSelect: () => adjustFontSize(-1),
      },
      ...
      },
      {
        ...
        onSelect: () => {
          ...
          notifications.push(
            `${wasEnabled ? language.t("toast.workspace.disabled.title") : language.t("toast.workspace.enabled.title")}: ${wasEnabled ? language.t("toast.workspace.disabled.description") : language.t("toast.workspace.enabled.description")}`,
            "toast",
          )
        },
      },
  ...
  const deleteWorkspace = async (root: string, directory: string, leaveDeletedWorkspace = false) => {
    ...
    const result = await globalSDK.client.worktree
      ...
      .catch((err) => {
        notifications.push(
          `${language.t("workspace.delete.failed.title")}: ${errorMessage(err, language.t("common.requestFailed"))}`,
          "toast",
        )
        return false
      })
  ...
  const resetWorkspace = async (root: string, directory: string) => {
    ...
    notifications.push(
      `${language.t("workspace.resetting.title")}: ${language.t("workspace.resetting.description")}`,
      "toast",
    )
    ...
    const result = await globalSDK.client.worktree
      ...
      .catch((err) => {
        notifications.push(
          `${language.t("workspace.reset.failed.title")}: ${errorMessage(err, language.t("common.requestFailed"))}`,
          "toast",
        )
        return false
      })
    ...
    notifications.push(
      `${language.t("workspace.reset.success.title")}: ${language.t("workspace.reset.success.description")}`,
      "toast",
    )
  }
  ...
  createEffect(
    on(
      () => [pageReady(), currentDir(), params.id] as const,
      ([ready, directory, sessionID]) => {
        if (!ready || !directory) return

        if (!sessionID) {
          if (activeLane.directory === directory && !activeLane.sessionID) return
          activeLane.directory = directory
          activeLane.sessionID = ""
          void globalSDK.client.session.laneDeactivate({ directory }).catch(() => {})
          return
        }

        if (activeLane.directory === directory && activeLane.sessionID === sessionID) return
        activeLane.directory = directory
        activeLane.sessionID = sessionID
        void globalSDK.client.session.laneActivate({ sessionID, directory }).catch(() => {})
      },
    ),
  )
  ...
  const createWorkspace = async (project: LocalProject) => {
    ...
    const created = await globalSDK.client.worktree
      ...
      .catch((err) => {
        notifications.push(
          `${language.t("workspace.create.failed.title")}: ${errorMessage(err, language.t("common.requestFailed"))}`,
          "toast",
        )
        return undefined
      })
    ...
    return (
      <div
        ...
        classList={{
          ...
          "bg-sidebar": merged() || hover(),
          "bg-elevated": !merged() && !hover(),
  ...
  onMount(() => {
    ...
    requestAnimationFrame(() => {
      animate(viewRef as Element, { opacity: [0, 1], y: [4, 0] }, {
        type: "spring",
        stiffness: 400,
        damping: 30,
      } as any)
  ...
  return (
    <ActiveTaskProvider>
      <div class="relative bg-canvas flex-1 min-h-0 min-w-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text" data-ui-region="canvas">
        <Titlebar />
        <div class="flex-1 min-h-0 min-w-0 flex overflow-hidden">
          {/* VSCode-style Activity Bar - Far Left */}
          <Show when={store.activityBarVisible}>
            <div class=" hidden w-12 shrink-0 bg-chrome flex flex-col items-center py-2 gap-2 z-20" data-ui-region="chrome">
              {/* Project Icons Rail - VSCode Style */}
              <div class="flex-1 w-full flex flex-col items-center gap-1 overflow-y-auto no-scrollbar">
                <For each={layout.projects.list()}>
                  {(project: LocalProject) => {
                    const selected = createMemo(() => currentProject()?.worktree === project.worktree)
                    return (
                      <button
                        type="button"
                        class="flex items-center justify-center size-9 p-0.5 rounded-md transition-colors"
                        classList={{
                          "bg-transparent hover:bg-surface-base-hover": !selected(),
                          "bg-surface-base-hover border border-border-weak-base": selected(),
                        }}
                        onClick={() => navigate(`/${base64Encode(project.worktree)}/session`)}
                        aria-label={project.name || getFilename(project.worktree)}
                      >
                        <ProjectIcon project={project} notify />
                      </button>
                    )
                  }}
                </For>
                <button
                  type="button"
                  class="flex items-center justify-center size-9 p-1 rounded-md transition-colors hover:bg-surface-base-hover"
              ...
              {/* Bottom Actions */}
              <div class="shrink-0 w-full flex flex-col items-center gap-1 pb-2">
                <button
                  type="button"
                  class="flex items-center justify-center size-9 p-1 rounded-md transition-colors hover:bg-surface-base-hover"
            ...
            <div class="size-full relative overflow-x-hidden">
              ...
              <div class="xl:hidden">
                ...
                <nav
                  ...
                  classList={{
                    "@container fixed top-10 bottom-0 left-0 z-50 w-full max-w-[400px] overflow-hidden border-r border-border-weaker-base bg-sidebar transition-transform duration-200 ease-out": true,
              ...
              <div
                classList={{
                  "absolute p-3 pt-0 inset-0": true,
                  "xl:inset-y-0 xl:right-0 xl:left-0": true,
                  "z-20": true,
                }}
              >
                <main
                  classList={{
                    "size-full overflow-x-hidden flex flex-col items-start contain-strict rounded-xl overflow-hidden": true,
                    "pb-8": isCloudMode(),
                  }}
                >
                  <Show when={!autoselecting.loading} fallback={<div class="size-full" />}>
                    <div class="size-full" ref={viewRef}>
                      {props.children}
                    </div>
                  </Show>
                </main>
              ...
              {/* Peek panels removed - not needed with new layout */}
            </div>
          </div>
        </div>
      </div>
