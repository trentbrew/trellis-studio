...
export namespace ACP {
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    if (opencodeProvider) {
      if (opencodeProvider.models["minimax-m3-free"]) {
        return { providerID: ProviderID.opencode, modelID: ModelID.make("minimax-m3-free") }
      }
      if (opencodeProvider.models["minimax-m2.5-free"]) {
        return { providerID: ProviderID.opencode, modelID: ModelID.make("minimax-m2.5-free") }
      }
      if (opencodeProvider.models["nemotron-3-super-free"]) {
        return { providerID: ProviderID.opencode, modelID: ModelID.make("nemotron-3-super-free") }
      }
    ...
    const trellisCloud = providers.find((p) => p.id === "trellis-cloud")
    if (trellisCloud?.models["gemini-flash-lite-latest"]) {
      return {
        providerID: ProviderID.make("trellis-cloud"),
        modelID: ModelID.make("gemini-flash-lite-latest"),
      }
    }
