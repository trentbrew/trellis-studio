...
export namespace Config {
    ...
    .preprocess(
      ...
      z
        .object({
          ...
          deep_research: PermissionAction.optional(),
  ...
  export const PreviewServiceType = z.enum(["web", "api", "terminal"]).meta({
    ref: "PreviewServiceType",
  })
  export type PreviewServiceType = z.infer<typeof PreviewServiceType>

  export const PreviewService = z
    .object({
      port: z
        .number()
        .int()
        .min(0)
        .optional()
        .describe("Port the service listens on. 0 or omitted means no HTTP port."),
      type: PreviewServiceType.optional()
        .default("web")
        .describe("Rendering mode: web (iframe), api (REST client), or terminal"),
      command: z.string().describe("Shell command to start the service"),
      url: z.string().optional().describe("URL override, e.g. http://localhost:3000/app"),
      env: z.record(z.string(), z.string()).optional().describe("Extra environment variables"),
    })
    .strict()
    .meta({
      ref: "PreviewServiceConfig",
    })
  export type PreviewService = z.infer<typeof PreviewService>

  export const Preview = z
    .object({
      services: z
        .record(z.string(), PreviewService)
        .optional()
        .describe("Named dev services with port, type, and start command"),
    })
    .strict()
    .meta({
      ref: "PreviewConfig",
  ...
  export const Capture = z
    .object({
      links: z
        .boolean()
        .optional()
        .describe("Auto-save http(s) URLs from user prompts as link assets in the Design library (default true)"),
      explicitMemory: z
        .boolean()
        .optional()
        .describe("Auto-save explicit 'remember …' phrases from user prompts to Trellis memory (default true)"),
      researchSources: z
        .boolean()
        .optional()
        .describe(
  ...
  export const Projections = z
    .object({
      workspaceType: z
        .string()
        .optional()
        .describe("Workspace type used to choose Trellis Studio projection rail defaults."),
      pinned: z.array(z.string()).optional().describe("Projection IDs pinned in the Studio projection rail."),
      hidden: z.array(z.string()).optional().describe("Projection IDs hidden from the Studio projection picker."),
    })
    .strict()
    .meta({
      ref: "ProjectionsConfig",
    })
  ...
  export const Info = z
    .object({
      ...
      env: z.record(z.string(), z.string()).optional().describe("Environment variables scoped to this config"),
      ...
      preview: Preview.optional().describe("Preview service configuration for dev servers"),
      capture: Capture.optional().describe("Automatic capture of URLs and explicit memory from user prompts"),
      projections: Projections.optional().describe("Trellis Studio projection rail defaults and overrides"),
      media_analysis: z
        .object({
          image: z
            .object({
              policy: z.enum(["auto", "local", "cloud", "off"]).optional(),
