    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const getBrewFormula = Effect.fnUntraced(function* () {
          const tapFormula = yield* text(["brew", "list", "--formula", "trentbrew/tap/turtlecode"])
          if (tapFormula.includes("turtlecode")) return "trentbrew/tap/turtlecode"
        ...
        const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
          ...
          const response = yield* httpOk.execute(
            HttpClientRequest.get("https://api.github.com/repos/trentbrew/trellis-studio/releases/latest").pipe(
              HttpClientRequest.acceptJson,
            ),
          )
        ...
        const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
          ...
          switch (m) {
            ...
            case "brew": {
              ...
              if (formula.includes("/")) {
                const tap = yield* run(["brew", "tap", "trentbrew/tap"], { env })
                if (tap.code !== 0) {
                  result = tap
                  break
                }
                const repo = yield* text(["brew", "--repo", "trentbrew/tap"])
                const dir = repo.trim()
                if (dir) {
