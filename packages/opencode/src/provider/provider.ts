...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async "trellis-cloud"(input) {
      const auth = await Auth.get(TRELLIS_CLOUD_PROVIDER)
      const envToken = Env.get("TRELLIS_CLOUD_AUTH")
      const apiKey = auth?.type === "api" ? auth.key : envToken
      if (!apiKey) return { autoload: false }

      const runtimeConfig = await Config.get()
      const configuredBaseURL = runtimeConfig.provider?.[TRELLIS_CLOUD_PROVIDER]?.options?.baseURL
      const brokerUrl = iife(() => {
        if (typeof configuredBaseURL === "string" && configuredBaseURL.trim() !== "") {
          return configuredBaseURL.replace(/\/ai\/v1\/?$/, "")
        }
        return Env.get("TRELLIS_BROKER_URL") ?? TRELLIS_CLOUD_DEFAULT_BROKER
      })

      return {
        autoload: true,
        options: {
          baseURL: `${brokerUrl.replace(/\/+$/, "")}/ai/v1`,
          apiKey,
          includeUsage: true,
        },
      }
  ...
  const TRELLIS_CLOUD_PROVIDER = "trellis-cloud" as const
  const TRELLIS_CLOUD_MODEL = "gemini-flash-lite-latest" as const
  const TRELLIS_CLOUD_DEFAULT_BROKER = "https://api.studio.trellis.computer"

  function injectTrellisCloudProvider(database: Record<string, Info>) {
    if (database[TRELLIS_CLOUD_PROVIDER]) return
    const providerID = ProviderID.make(TRELLIS_CLOUD_PROVIDER)
    const modelID = ModelID.make(TRELLIS_CLOUD_MODEL)
    database[TRELLIS_CLOUD_PROVIDER] = {
      id: providerID,
      source: "custom",
      name: "Trellis Cloud",
      env: ["TRELLIS_CLOUD_AUTH"],
      options: {},
      models: {
        [TRELLIS_CLOUD_MODEL]: {
          id: modelID,
  ...
  const state = Instance.state(async () => {
    ...
    injectTrellisCloudProvider(database)
  ...
  // Later entries win (sort uses findIndex descending). Nemotron before big-pickle in the list would rank big-pickle higher.
  const priority = ["gpt-5", "claude-sonnet-4", "big-pickle", "nemotron-3-super-free", "gemini-3-pro"]
