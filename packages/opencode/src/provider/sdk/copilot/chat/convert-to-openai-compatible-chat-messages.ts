            ...
            switch (part.type) {
              ...
              case "file": {
                if (part.mediaType.startsWith("image/")) {
                  ...
                  // Gracefully handle ambiguous MIME types (e.g., .ts files reported as video/mp2t)
                  // Convert to text content instead of throwing
                  const textContent =
                    typeof part.data === "string"
                      ? part.data
                      : part.data instanceof URL
                        ? part.data.toString()
                        : Buffer.isBuffer(part.data) || part.data instanceof Uint8Array
                          ? new TextDecoder().decode(part.data)
                          : String(part.data)
                  return { type: "text", text: textContent, ...partMetadata }
                }
              }
            }
          }),
