...
export const SessionRoutes = lazy(() =>
  new Hono()
    ...
    .post(
      "/:sessionID/lane/activate",
      describeRoute({
        summary: "Activate session lane",
        description: "Enter the Trellis lane bound to this session when switching session tabs.",
        operationId: "session.laneActivate",
        responses: {
          200: {
            description: "200",
            content: {
      ...
      validator(
        "param",
        z.object({
          sessionID: Session.activateLane.schema,
        }),
      ),
      async (c) => {
        const sessionID = c.req.valid("param").sessionID
        const result = await Session.activateLane(sessionID)
        return c.json(result)
      },
    )
    .post(
      "/lane/deactivate",
      describeRoute({
        summary: "Deactivate session lane",
        description: "Leave the active Trellis lane when no session tab is selected.",
        operationId: "session.laneDeactivate",
        responses: {
          200: {
            description: "200",
            content: {
