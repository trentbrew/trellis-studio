      ...
      .use(
        cors({
          ...
          origin(input) {
            ...
            // Allow private/local network IPs (RFC 1918)
            if (/^http:\/\/(10\.|172\.(1[6-9]|2\d|3[01])\.|192\.168\.)/.test(input)) return input
            if (
            ...
            // E2B sandbox Studio (turtlecode on port 3333)
            if (/^https:\/\/[a-z0-9-]+.*\.e2b\.(app|dev|pro)$/.test(input)) {
              return input
            }
