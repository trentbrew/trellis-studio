...
export namespace Session {
  ...
  export function fromRow(row: SessionRow): Info {
    ...
    return {
      ...
      laneForkKind: row.lane_fork_kind ?? undefined,
      laneUnpromotedParent: undefined,
      title: row.title,
      version: row.version,
      summary,
      share,
  ...
  export function toRow(info: Info) {
    return {
      ...
      lane_id: info.laneID,
      parent_lane_id: info.parentLaneID,
      lane_fork_kind: info.laneForkKind,
      slug: info.slug,
  ...
  export const Info = z
    .object({
      ...
      laneForkKind: z.enum(["sibling", "child"]).optional(),
      laneUnpromotedParent: z.boolean().optional(),
      summary: z
        .object({
          additions: z.number(),
          deletions: z.number(),
  ...
  export interface Interface {
    ...
    readonly activateLane: (sessionID: SessionID) => Effect.Effect<{ laneID?: string }>
    readonly deactivateLane: () => Effect.Effect<void>
  ...
  export const layer: Layer.Layer<Service, never, Bus.Service | Config.Service> = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const createNext = Effect.fn("Session.createNext")(function* (input: {
        ...
        if (input.bindLane !== false) {
          ...
          if (lane) {
            ...
            yield* Effect.sync(() =>
              SyncEvent.run(Event.Updated, { sessionID: result.id, info: { laneID: lane.laneID } }),
            )
          }
        }
      ...
      const activateLane = Effect.fn("Session.activateLane")(function* (sessionID: SessionID) {
        const session = yield* get(sessionID)
        const binding = yield* Effect.promise(() =>
          SessionLane.activateForSession({
            sessionID,
            directory: session.directory,
            laneID: session.laneID,
          }),
        )
        if (binding?.laneID && binding.laneID !== session.laneID) {
          yield* patch(sessionID, { laneID: binding.laneID })
        }
        return { laneID: binding?.laneID }
      })

      const deactivateLane = Effect.fn("Session.deactivateLane")(function* () {
        yield* Effect.promise(() => SessionLane.deactivate(Instance.directory))
      })
      ...
      const fork = Effect.fn("Session.fork")(function* (input: { sessionID: SessionID; messageID?: MessageID }) {
        ...
        const binding = yield* Effect.promise(() =>
          SessionLane.fork({ parent: original, childSessionID: session.id, kind: "child" }),
        )
        if (!binding) return session

        session.laneID = binding.laneID
        session.parentLaneID = binding.parentLaneID
        session.laneForkKind = binding.forkKind
        if (binding.unpromotedParent) session.laneUnpromotedParent = true
        yield* Effect.sync(() =>
          SyncEvent.run(Event.Updated, {
            sessionID: session.id,
            info: {
              laneID: binding.laneID,
              parentLaneID: binding.parentLaneID,
              laneForkKind: binding.forkKind,
            },
          }),
        )
        return session
  ...
  export const activateLane = fn(SessionID.zod, (sessionID) => runPromise((svc) => svc.activateLane(sessionID)))

  export const deactivateLane = fn(z.void(), () => runPromise((svc) => svc.deactivateLane()))
