      ...
      if (msg.info.role === "user") {
        ...
        for (const part of msg.parts) {
          ...
          if (
            ...
            if (!isMedia(part.mime) && DocumentExtractor.supported(part.mime)) {
              userMessage.parts.push({
                type: "text",
                text: `ERROR: Cannot read "${part.filename ?? "document"}" (document text extraction failed). Inform the user.`,
              })
              continue
            }
