...
test("loads top-level env into instance env", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode.ai/config.json",
        env: {
          TRELLIS_ORIGIN: "opencode",
          TRELLIS_ROOT: ".",
        },
      })
    },
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(Env.get("TRELLIS_ORIGIN")).toBe("opencode")
      expect(Env.get("TRELLIS_ROOT")).toBe(".")
    },
  })
})
...
test("loads research command with agent and subtask frontmatter", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(commandDir, "research.md"),
        `---
...
$ARGUMENTS`,
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(config.command?.["research"]).toMatchObject({
        description: "Research a topic on the web with citations (quick, deep, or exhaustive)",
        agent: "research",
        subtask: true,
      })
      expect(String(config.command?.["research"]?.template)).toContain("/research")
      expect(String(config.command?.["research"]?.template)).toContain("$ARGUMENTS")
    },
  })
})
