...
describe("file/index Filesystem patterns", () => {
  ...
  describe("File.read() - binary content", () => {
    ...
    test("reads audio files as base64 preview content", async () => {
      await using tmp = await tmpdir()
      const bytes = Buffer.from([0x52, 0x49, 0x46, 0x46])
      await fs.writeFile(path.join(tmp.path, "tone.wav"), bytes)

      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          const result = await File.read("tone.wav")
          expect(result.type).toBe("text")
          expect(result.encoding).toBe("base64")
  ...
  describe("File.search()", () => {
    ...
    test("space-separated query matches path segments", async () => {
      await using tmp = await setupSearchableRepo()
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const result = await File.search({ query: "src main", type: "file" })
          expect(result.some((f) => f.includes("src/main"))).toBe(true)
        },
      })
    })
