...
describe("ProviderTransform.schema - strict object (DeepSeek / Big Pickle)", () => {
  const deepseekModel = {
    id: "big-pickle",
    providerID: "opencode",
    api: {
  ...
  test("converts discriminated oneOf to root type object", () => {
    const schema = {
      oneOf: [
        {
          type: "object",
          properties: {
            action: { type: "string", enum: ["create_link_asset"] },
            url: { type: "string" },
          },
          required: ["action", "url"],
        },
    ...
    expect(result.properties.action.enum).toEqual(
      expect.arrayContaining(["create_link_asset", "generate_image_asset"]),
    )
    expect(result.properties.url).toBeDefined()
    expect(result.properties.prompt).toBeDefined()
    expect(result.type).not.toBeNull()
  })

  test("strips type null nodes", () => {
    const schema = {
      type: null,
      properties: {
        command: { type: "string" },
    ...
    expect(result.type).toBe("object")
    expect(result.properties.command.type).toBe("string")
  })
})
