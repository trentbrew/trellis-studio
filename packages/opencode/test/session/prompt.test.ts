...
function imageConfig(input: boolean) {
  const modes: ("text" | "image")[] = input ? ["text", "image"] : ["text"]
  return {
    enabled_providers: ["debug"],
    provider: {
      debug: {
        name: "Debug",
        npm: "@ai-sdk/openai",
        ...
        models: {
          model: {
            ...
            modalities: { input: modes, output: ["text" as const] },
            options: {},
          },
        },
      },
...
describe("session.prompt image attachments", () => {
  test("explains when unavailable analysis drops image for non-vision model", async () => {
    await using tmp = await tmpdir({ git: true, config: imageConfig(false) })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(text.some((part) => part.includes("not marked as capable of reading it"))).toBe(true)
  ...
  test("keeps image when analysis is unavailable but model is vision capable", async () => {
    await using tmp = await tmpdir({ git: true, config: imageConfig(true) })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const text = msg.parts.filter((part) => part.type === "text").map((part) => part.text)
        const file = msg.parts.find((part) => part.type === "file")
        expect(text.some((part) => part.includes("not marked as capable of reading it"))).toBe(false)
...
describe("session.prompt attachment focus", () => {
  test("prioritizes inline image attachments over ambient whiteboard context", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        await Bun.write(
          path.join(dir, "board.whiteboard"),
          `${JSON.stringify({
            type: "excalidraw",
            version: 2,
            elements: [{ id: "a", type: "freedraw", points: [[0, 0], [1, 1]], isDeleted: false }],
            appState: {},
            files: {},
          })}\n`,
        )
      },
      config: imageConfig(false),
    })

    MediaAnalysis.configure({
      local: async () => ({ text: "A person playing guitar", source: "local", elapsed: 0 }),
    })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const session = await Session.create({})
        const board = path.join(tmp.path, "board.whiteboard")
        const msg = await SessionPrompt.prompt({
          sessionID: session.id,
          agent: "build",
          noReply: true,
        ...
        expect(text).toContain("<attachment-focus>")
        expect(text).toContain("A person playing guitar")
        expect(text).not.toContain("<whiteboard-tool>")
        expect(text).toContain("prioritize those unless they ask about the whiteboard")
        expect(text).not.toContain('"type": "freedraw"')
