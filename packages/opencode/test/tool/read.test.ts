...
describe("tool.read truncation", () => {
  ...
  test("image files set truncated to false", async () => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        MediaAnalysis.configure({ local: async () => ({ text: "red pixel", source: "local", elapsed: 0 }) })
        const read = await ReadTool.init()
        const result = await read.execute({ filePath: path.join(tmp.path, "image.png") }, ctx)
        expect(result.metadata.truncated).toBe(false)
        expect(result.output).toContain("Target Identifier: image.png")
        expect(result.output).toContain("red pixel")
        expect(result.attachments).toBeUndefined()
      },
    })
  })

  test("whiteboard files return semantic describe instead of raw json", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          path.join(dir, "sketch.whiteboard"),
          `${JSON.stringify({
            type: "excalidraw",
            version: 2,
            elements: [{ id: "a", type: "freedraw", isDeleted: false, points: [[0, 0]] }],
          })}\n`,
        )
        await Bun.write(
          path.join(dir, ".trellis/renders/sketch.meta.json"),
          `${JSON.stringify({
            version: 1,
            whiteboardPath: "sketch.whiteboard",
            elementsKey: "1:a",
        ...
        await Bun.write(path.join(dir, ".trellis/renders/sketch.png"), png)
      },
    })
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        MediaAnalysis.configure({ local: async () => ({ text: "handwritten hello", source: "local", elapsed: 0 }) })
        const read = await ReadTool.init()
        const result = await read.execute({ filePath: path.join(tmp.path, "sketch.whiteboard") }, ctx)
        expect(result.output).toContain("freedraw × 1")
        expect(result.output).toContain(".trellis/renders/sketch.png")
        expect(result.output).toContain("handwritten hello")
        expect(result.output).not.toContain('"points"')
      },
    })
  })

  test("large image files are analyzed without attaching raw binary", async () => {
    await Instance.provide({
      directory: FIXTURES_DIR,
      fn: async () => {
        MediaAnalysis.configure({ local: async () => ({ text: "large image", source: "local", elapsed: 0 }) })
        const read = await ReadTool.init()
        const result = await read.execute({ filePath: path.join(FIXTURES_DIR, "large-image.png") }, ctx)
        expect(result.metadata.truncated).toBe(false)
        expect(result.output).toContain("large image")
        expect(result.attachments).toBeUndefined()
      },
    })
  })

  test("audio files are analyzed without attaching raw binary", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "voice.mp3"), Buffer.from("audio"))
      },
    })
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        MediaAnalysis.configure({ local: async () => ({ text: "short transcript", source: "local", elapsed: 0 }) })
        const read = await ReadTool.init()
        const result = await read.execute({ filePath: path.join(tmp.path, "voice.mp3") }, ctx)
        expect(result.metadata.truncated).toBe(false)
        expect(result.output).toContain("Attachment Type: audio")
        expect(result.output).toContain("short transcript")
        expect(result.attachments).toBeUndefined()
      },
    })
  })
