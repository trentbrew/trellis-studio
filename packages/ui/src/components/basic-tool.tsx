...
const GENERIC_TOOL_ICONS: Record<string, IconProps["name"]> = {
  memory: "brain",
}

function clipGeneric(value: string, limit = 72) {
...
export function GenericTool(props: {
  ...
  const i18n = useI18n()
  const icon = () => GENERIC_TOOL_ICONS[props.tool] ?? "mcp"
  const subtitle = () => {
    const primary = label(props.input)
    if (primary) return clipGeneric(primary)
    const extra = args(props.input)
    if (extra.length) return clipGeneric(extra.join(" · "), 96)
    return undefined
  }
