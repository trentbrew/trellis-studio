...
export const SessionReview = (props: SessionReviewProps) => {
  ...
  const [store, setStore] = createStore({
    ...
    count: step,
    loader: undefined as HTMLDivElement | undefined,
  })
  ...
  const comments = createMemo(() => {
    const out = new Map<string, SessionReviewComment[]>()
    for (const comment of props.comments ?? []) {
      const list = out.get(comment.file)
      if (list) {
        list.push(comment)
        continue
      }

      out.set(comment.file, [comment])
    }
    return out
  })
  const shown = createMemo(() => files().slice(0, store.count))
  const index = createMemo(() => new Map(files().map((file, at) => [file, at] as const)))
  const openSet = createMemo(() => new Set(open()))
  const diffStyle = () => props.diffStyle ?? (props.split ? "split" : "unified")
  const hasDiffs = () => files().length > 0
  const more = () => store.count < files().length

  const ensure = (file: string) => {
    const at = index().get(file)
    if (at === undefined) return
    if (at < store.count) return

    setStore("count", Math.min(files().length, at + step))
  }

  const load = () => {
    if (!more()) return
    setStore("count", Math.min(files().length, store.count + step))
  }

  const handleScroll: JSX.EventHandlerUnion<HTMLDivElement, Event> = (event) => {
    const scroll = props.onScroll
    if (typeof scroll === "function") scroll(event)
  ...
  createEffect(() => {
    const root = scroll
    const el = store.loader
    if (!root || !el || !more() || typeof IntersectionObserver === "undefined") return
  ...
  createEffect(() => {
    if (!props.focusedFile) return
    ensure(props.focusedFile)
  })
              ...
              <Accordion multiple value={open()} onChange={handleChange}>
                <For each={shown()}>
                  {(file) => {
                    ...
                    const commentsForFile = createMemo(() => comments().get(file) ?? [])
                    const commentedLines = createMemo(() => commentsForFile().map((c) => c.selection))
