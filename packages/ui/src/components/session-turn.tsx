...
export function SessionTurn(
  ...
  const showThinking = createMemo(() => {
    ...
    const visibleCount = assistantVisible()

    if (!isWorking || hasError) return false
    if (statusType === "retry") return false
    if (showReasoning) return visibleCount === 0
    return true
  })
  ...
  return (
    <div data-component="session-turn" class={props.classes?.root} data-active={active() ? "" : undefined}>
      <div
      ...
      >
        <div onClick={autoScroll.handleInteraction}>
          <Show when={message()}>
            ...
            >
              <div data-slot="session-turn-message-content" class="px-4 md:px-5" aria-live="off">
                <Message
                  message={message()!}
                  parts={parts()}
              ...
              <Show when={assistantMessages().length > 0}>
                <div
                  ...
                  class="w-full px-4 md:px-5 box-border"
                  aria-busy={working()}
                >
                  <AssistantParts
