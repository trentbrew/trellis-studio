...
export function resolveThemeVariant(variant: ThemeVariant, isDark: boolean): ResolvedTheme {
  ...
  tokens["bg-canvas"] = neutral[0]
  tokens["bg-chrome"] = neutral[1]
  tokens["bg-sidebar"] = neutral[2]
  tokens["bg-panel"] = neutral[0]
  tokens["bg-well"] = neutral[0]
  tokens["bg-subtle"] = neutralAlpha[0]
  tokens["bg-elevated"] = isDark ? neutral[1] : ("#fcfcfc" as HexColor)
  tokens["bg-overlay"] = isDark ? neutral[1] : neutral[11]
  tokens["bg-scrim"] = withAlpha(neutral[0], isDark ? 0.55 : 0.35) as ColorValue

  tokens["background-base"] = tokens["bg-canvas"]
  tokens["background-weak"] = tokens["bg-sidebar"]
  tokens["background-strong"] = tokens["bg-panel"]
  tokens["background-stronger"] = tokens["bg-elevated"]
  ...
  syncElevationLegacy(tokens, overrides)

  return tokens
}

/** Keep ladder steps and legacy background-* aliases aligned after overrides. */
function syncElevationLegacy(tokens: ResolvedTheme, overrides: Record<string, ColorValue>): void {
  const pairs: Array<[ladder: string, legacy: string]> = [
    ["bg-canvas", "background-base"],
    ["bg-sidebar", "background-weak"],
    ["bg-panel", "background-strong"],
    ["bg-elevated", "background-stronger"],
  ]

  for (const [ladder, legacy] of pairs) {
    if (legacy in overrides && !(ladder in overrides)) {
      tokens[ladder] = tokens[legacy]
    }
    if (ladder in overrides && !(legacy in overrides)) {
      tokens[legacy] = tokens[ladder]
    }
  }
}
